...
self.runCommand = () => {
  ...
  // cd into command.
    ...
    if (input[0] === 'help') {
      self.eol();
      self.callCommand('help', self.breadcrumbs.join('.'));
      return;
    }
    ...
    let isMetaCommand = self.metaCommands.get(commandKey);
    if (isMetaCommand) {
  ...
  // command [command] [value] [--flags]
    ...
    if (commandKey) {
      ...
      let commandArgsOffset = commandKey.split('.').length - self.breadcrumbs.length;
