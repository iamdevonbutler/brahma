...
process.stdin.on('keypress', function (ch, key) {
  ...
  if (key && key.ctrl && key.name === 'c') {
    ...
    // if (self.cursor.pos() > self.getDelimiterLength()) {
      // let pos = self.getLineLength() - self.cursor.pos();
      // if (pos === 0) {
    ...
    // var pos = self.getLineLength() - self.cursor.pos();
    // if (pos > 0) {
    //   if (pos === 1) {
    //     self.buffer = self.buffer.slice(0, -1);
    //   }
    //   else {
    //     self.buffer = self.buffer.slice(0, self.buffer.length - pos) + buffer.slice(self.buffer.length - pos + 1);
    //   }
    //   term.delete(-1);
    // }
  }
