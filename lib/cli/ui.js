...
process.stdin.on('data', function (data) {
  ...
  if (data) {
    ...
    term.left(self.cursorXIndex);
    term.right(self.getDelimiterLength() + startPos + 1 - (self.getLineLength() - self.cursorXIndex));
  }
});
...
process.stdin.on('keypress', function (ch, key) {
  ...
  if (key && key.ctrl && key.name === 'c') {
    ...
    // term.left(self.cursorXIndex);
    return;
    // Space deletes characters by default.
    if (self.cursorXIndex < self.getLineLength()) {
      let startPos = self.cursorXIndex - self.getDelimiterLength() - 1;
      // console.log(startPos);
      self.buffer = self.buffer.slice(0, startPos) + ' ' + self.buffer.slice(startPos);
      self.cursorXIndex += 1;
      term.deleteLine();
      term.left(self.cursorXIndex);
      self.cursorXIndex = 0;
