...
const validateCommands = require('./validate');

// @todos
// command "status" property. run status as a precursor to a command main method. choose specific status checks to run and be able to run all of them.
...
self.initTerminal = (commandsPath, cachePath) => {
  ...
  // var err = validateCommands(commands.keys(), metaCommands.keys());
  // console.log(1111, err);
  // if (err) throw new Error(`Command validation error: ${err}`);
