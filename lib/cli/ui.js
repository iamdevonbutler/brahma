...
// @todos
// directories for resources and commands (index.js) can have a name field.
...
self.callCommand = (commandName, input, args = []) => {
  ...
  if (!command || !command.main) return;
...
self.runCommand = () => {
  ...
  if (commandName) {
    self.history.add(self.breadcrumbs, self.buffer);
    self.callCommand(commandName, input);
    return;
  }
