...
// when servers go offline Brahma should know about it, have some email integration.

const phone = self.connect({
  // clients: [],
  ip: '',
  name: `router-${id}`
  lost() {
...
var {err, data, reply, meta} = await phone.call('router.emploees.getEmployees', {
  data,
  reply: true,
  timeout: 12000,
});

phone.answer('router.employees.getEmployees', ({reply, meta}, data) => {
  // do work.
  reply({data}); // true = stream
});

// how does the queueing work?
// replys are optional - just dont await.
