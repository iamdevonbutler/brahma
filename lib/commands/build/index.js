...
module.exports = ({config, settings, env, activeEnv, variables}) => async (args) => {
  ...
  promises = microservicesList.map(appName => {
    ...
    shell.exec(`cp -r ${path.join(boilerplatePath, 'lib/bootstrap')} ${path.join(appRoot, 'lib/.')}`, {async: true});

    // ./build/[appName]/lib/config/**
    shell.exec(`cp -r ${path.join(boilerplatePath, 'lib/config')} ${path.join(appRoot, 'lib/.')}`, {async: true});

    // ./build/[appName]/lib/middleware/**
    shell.exec(`cp -r ${path.join(boilerplatePath, 'lib/middleware')} ${path.join(appRoot, 'lib/.')}`, {async: true});

    // ./build/[appName]/lib/services/**
    shell.exec(`cp -r ${path.join(boilerplatePath, 'lib/services')} ${path.join(appRoot, 'lib/.')}`, {async: true});

    // ./build/[appName]/lib/utils/**
    shell.exec(`cp -r ${path.join(boilerplatePath, 'lib/utils')} ${path.join(appRoot, 'lib/.')}`, {async: true});

    // ./build/[appName]/lib/decorators/**
    shell.exec(`cp -r ${path.join(boilerplatePath, 'lib/decorators')} ${path.join(appRoot, 'lib/.')}`, {async: true});
    ...
    if (fileExists(eslintrcPath)) {
      shell.exec(`cp ${eslintrcPath} ${path.join(appRoot, '/.')}`, {async: true});
    }
    else {
    ...
    if (fileExists(jscsrcPath)) {
      shell.exec(`cp ${jscsrcPath} ${path.join(appRoot, '/.')}`, {async: true});
    }
    else {
