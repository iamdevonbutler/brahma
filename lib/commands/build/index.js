...
function installDependencies(cwd) {
  // @todo install w/ env!
  // var src = path.join(cwd, 'bin/install.js');
  // var c = fork(src), null, {cwd: src});
  // console.log(src);
// c.on('message', (m) => {
...
module.exports = ({config, settings, env, variables}) => async (args) => {
  ...
  microservicesList.forEach(appName => {
    ...
    if (fileExists(eslintrcPath)) {
      shell.exec(`cp ${eslintrcPath} ${path.join(buildPath, appName, '/.')}`);
    }
    else {
    ...
    if (fileExists(jscsrcPath)) {
      shell.exec(`cp ${jscsrcPath} ${path.join(buildPath, appName, '/.')}`);
    }
    else {
    ...
    // @note idk if we are using git for shit yet?
    ...
    // ./build/[appName] npm install
    console.log(`installing dependencies for "${appName}"...`);
    var src = path.join(buildPath, appName);
    writeFile('./bin/install.js', './bin/install.js', appName, {src});
    var child = fork(path.join(src, 'bin/install.js'));
    child.on('exit', () => {
      var dependencies = Object.keys(config[appName].dependencies || {});
      dependencies.forEach(dependency => {
        var dir = path.join(src, 'node_modules', dependency);
        if (!directoryExists(dir)) {
          console.error(`app.${appName}.dependencies.${dependency} not installed.`);
        }
      });
    });
  });
