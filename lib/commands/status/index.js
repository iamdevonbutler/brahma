...
const {checkmark, x} = require('../../cli/utils');
...
module.exports = {
  ...
  status: {
    test() {
      return {err: null, info: 'successsss'}
    }
  },
  main(obj) {
    var str, errors = 0;
      ...
      .forEach((item, key, $item) => {
        ...
        $item.forEach((item1, key1) => {
          ...
          if (result.err) {
            ...
            if (result.info) {
              str += '       ' + chalk.green(key1 + ': ' + result.info + ' ' + checkmark());
            }
            else {
    ...
    str += EOL;
    if (errors) {
      str += chalk.red('>>>  ' + x() + '  ' + errors + (errors === 1 ? ' Error' : ' Errors'))
    }
    else {
