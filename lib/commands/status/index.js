...
module.exports = ({config, settings, env, variables}) => async (args) => {
  ...
  // brahma.env.js.
  // Make sure if HTTP or HTTPS, the "PORT" env is set.
  ...
  apps.forEach(appName => {
    ...
    const {http, https} = config[appName];
    if (http || https) {
      let keys = Object.keys(env[appName]);
      keys.forEach(envName => {
        const {PORT} = env[appName][envName];
        if (!PORT) {
          bad(`The "PORT" property is required (env.${appName}.${envName}.PORT)`);
        }
        // Validate each field.
