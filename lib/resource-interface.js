...
/**
 * Wraps object w/ methods resembling common functional array methods.
 * @param  {Object} data
 * @return {Object} the new Object is cloned from the the `data` param
 * and breaks the reference.
 */
  ...
  obj.forEach = (cb, async = false) => {
    ...
    if (async) {
      return new Promise(async (resolve, reject) => {
        var promises = keys.map(async key => await cb(obj.data[key], key));
...
self.applyResourceInterface = (data, schema = {}) => {
  ...
  var obj = self.applyObjectInterface(data);
  obj.raw = objectAssign({}, data);
