...
{% if mongorules %}const mongodb = require('mongodb');{% endif %}
{% if mongorules %}const mongorules = require('mongorules');{% endif %}
const path = require('path');
const bodyParser = require('koa-bodyparser');
const devLogger = require('koa-logger');
const responseTime = require('koa-response-time');
{% if cors %}const cors = require('koa2-cors');{% endif %}
{% if basicAuth %}const basicAuth = require('koa-basic-auth');{% endif %}
{% if lcache %}const lcache = require('lru-cache');{% endif %}
{% if mcache %}const mcache = require('memory-cache');{% endif %}
{% if static %}const koaStatic = require('koa-static');{% endif %}

{% if mongorules %}const loadMongorules = require('./bootstrap/loadMongorules');{% endif %}
{% if http || https %}const loadRoutes = require('./bootstrap/loadRoutes');{% endif %}
...
const {NODE_ENV, APP_NAME, PORT, CONCURRENCY = 1} = process.env;
...
module.exports = (config) => async () => {
  ...
  // Log web concurrency.
  messages.push(`-> Concurrency: "${CONCURRENCY}"`);
